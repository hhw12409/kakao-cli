...
| 2026-09-01 | **카카오톡 창 자동 관리 + 오프라인 캐시** — serve 브리지가 카카오톡을 자동 실행/최소화 복원하고 종료 시 되돌림(`WindowControl`). 띄우지 못하면 공통부가 캐시된 방 목록·대화를 읽기 전용으로 표시(`◐ 캐시`), `/rooms`·`/switch` 라이브 재시도. 첫 실행 게이트에서 "카카오톡 미실행" 제거. 라이브 검증: 최소화→wake→복원 OK. **미해결(선행 이슈): KakaoTalk 26.5.0 `listRooms` 셀렉터(`_NS:63`) 불일치 → 실기기에선 캐시 모드로만 동작.** | adapters/macos/*, crates/kakao-core/src/{tui,db,adapter}, packaging/, docs/ | 사용자 — "카카오톡이 무조건 켜져 있어야 함, 현재 상태로는 쓸 수 없음" |
