...
enum SelfTest {
    ...
    static func run() -> Never {
        ...
        let messages = Parsers.messages(
            in: convWindow, selectors: selectors, myName: nil, windowMinX: 1000, now: now
        )
        check(messages.count == 5, "5 messages parsed (spacer skipped)")
        ...
        check(!messages[0].outgoing, "left-aligned bubble (win+60) -> incoming")
        ...
        check(messages[2].text == "넵 확인했습니다", "outgoing message body")
        check(messages[2].outgoing, "right-aligned bubble (win+220) -> outgoing")
        check(messages[2].sender == "", "outgoing message carries no sender label")
        check(messages[3].sender == "수빈", "new incoming sender run picked up")
        check(!messages[3].outgoing, "수빈 message is incoming")
        check(messages[3].at != messages[1].at, "multiline '1\\n오전 11:30' timestamp parsed (not inherited)")
        check(messages[4].kind == .unsupported && messages[4].text == "", "media cell -> unsupported")
