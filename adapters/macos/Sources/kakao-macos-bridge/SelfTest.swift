...
enum SelfTest {
    ...
    static func run() -> Never {
        ...
        let messages = Parsers.messages(in: convWindow, selectors: selectors, myName: nil, now: now)
        check(messages.count == 3, "3 messages parsed (spacer skipped)")
        ...
        check(messages[1].text == "확인 중입니다", "second message body")
        check(messages[1].at == messages[0].at, "time inherited when row has no timestamp label")
        check(messages[2].kind == .unsupported && messages[2].text == "", "media cell -> unsupported")
