...
1. 카카오톡 데스크톱 앱에 **한 번 로그인**해 둔다 (이후 kakao-cli 가 알아서 실행·복원한다.
   최소화·트레이로 닫아 둬도 된다).
...
- **카카오톡 창은 kakao-cli 가 관리한다.** 실행돼 있지 않으면 대신 실행하고, 최소화·숨김·
  트레이로 닫혀 있으면 세션 동안만 복원한다. `kakao-cli` 를 끄면 원래대로 되돌린다
  (다시 최소화, 우리가 실행했으면 종료 — `KAKAO_CLI_KEEP_KAKAO=1` 로 끄지 않게).
- 폴링을 위해 세션 동안 카카오톡 창은 (포커스 없이) 떠 있다. `/switch` 와 **매 전송**은
  카카오톡 창을 잠깐 앞으로 올린다 (접근성 방식의 본질적 제약).
- **카카오톡을 띄울 수 없으면**(로그인 안 됨 등) 마지막으로 동기화한 방 목록·대화를
  **읽기 전용(캐시)** 으로 보여 준다. 카카오톡이 준비되면 자동으로 연결된다.

**넘을 수 없는 선:** 카카오톡이 설치돼 있고 한 번은 로그인돼 있어야 한다. 프로세스가
아예 없으면 조작할 접근성 트리도 없다.
